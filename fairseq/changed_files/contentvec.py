...
class ContentvecConfig(FairseqDataclass):
    ...
    freeze_encoder_layers: int = field(
        default=0, metadata={"help": "number of transformer layers to freeze"}
    )
...
@register_model("contentvec", dataclass=ContentvecConfig)
class ContentvecModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: ContentvecConfig,
        task_cfg: ContentvecPretrainingConfig,
        ...
        if cfg.freeze_encoder_layers > 0:
            logger.info(f"Freezing the first {cfg.freeze_encoder_layers} transformer layers.")
            for i in range(cfg.freeze_encoder_layers):
    ...
    @classmethod
    def build_model(cls, cfg: ContentvecConfig, task: ContentvecPretrainingTask):
        """Build a new model instance."""

        model = ContentvecModel(cfg, task.cfg, task.dictionaries)
